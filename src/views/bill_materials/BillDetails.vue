...
const openDialog = async (billIDAt: number) => {
  billID.value = billIDAt;
  await billMatStore.getBillItemsAt(billIDAt);
  dialog.value = true;
...
// function save() {
//   billMatStore.saveBillItems(billID.value);
// }
</script>
            ...
            <v-row>
              <v-col cols="3" offset="9">
                <v-btn
                  ...
                  @click="billMatStore.dialogD = true"
                  >Add New</v-btn
                >
              </v-col>
            </v-row>
            <v-row>
              <v-col>
                ...
                >
                  ...
                  <tbody>
                    <tr
                      v-for="item of billMatStore.billMaterialsAt?.billItems"
                      :key="item.id"
                    >
                      <td>{{ item.id }}</td>
                      <td>{{ item.name }}</td>
                      ...
                      <td>
                        <v-btn
                          ...
                          @click="billMatStore.editBillItem(item)"
                          >Edit</v-btn
                        >
                        <v-btn
                          :prepend-icon="mdiDelete"
                          class="vBtnCard button-3d styleBtnRed fontBtn"
                          @click="billMatStore.deleteBillItem(item.id!)"
                          >Delete</v-btn
                        >
                      </td>
                    </tr>
