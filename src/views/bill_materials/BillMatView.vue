...
import BillMatDialog from "./BillMatDialog.vue";
...
onMounted(async () => {
  await billMatStore.getBillMaterials();
});
...
const orderList = computed(() => {
  return [...billMatStore.billMaterials].sort((n1, n2) => n2.id! - n1.id!); //reverse);
});
...
<template>
  <!-- <div>BillMaterial</div>
  <div>{{ billMatStore.billMaterials }}</div> -->
  <BillMatDialog></BillMatDialog>
  <v-container>
    <v-row>
      <v-col cols="6" offset="1">
        <AutoCompleteFind
          label="Search Bill Material"
          :nameLists="billMatStore.nameBillMaterial()"
        ></AutoCompleteFind
      ></v-col>
      <v-col cols="2" offset="2">
        <v-btn
          ...
          @click="billMatStore.dialog = true"
          >Add New</v-btn
        >
      </v-col>
    </v-row>
          ...
          <v-table :style="varcpStore.styleTable" fixed-header height="70vh">
            <thead>
              <tr>
                ...
                <th :style="varcpStore.styleTableTh">Shop Name</th>
                <th :style="varcpStore.styleTableTh">Date</th>
                <th :style="varcpStore.styleTableTh">Total</th>
                <th :style="varcpStore.styleTableTh">Buy</th>
                <th :style="varcpStore.styleTableTh">Change</th>
            ...
            <tbody>
              <tr v-for="item of orderList" :key="item.id">
                <td
                  v-if="
                    varcpStore.valuesMatAutocom == item.shop_name ||
                ...
                >
                  ...
                  <v-btn
                    ...
                    @click="billMatStore.editBillMaterial(item)"
                    >Edit</v-btn
                  >
                  <v-btn
                    :prepend-icon="mdiDelete"
                    class="vBtnCard button-3d styleBtnRed fontBtn"
                    @click="billMatStore.deleteBill(item.id!)"
                    >Delete</v-btn
                  >
                </td>
              </tr>
  ...
  <BillDetails ref="detailDialog"></BillDetails>
</template>
<style scoped>
.button-3d {
  position: relative;
      ...
      <v-col cols="12">
        ...
        >
          <v-expansion-panels>
            ...
            <v-expansion-panel
              v-for="item of billMatStore.billMaterials"
              :key="item.id"
                ...
                <v-row no-gutters>
                  ...
                  <v-col class="text--secondary text-center">
                    <v-fade-transition leave-absolute>
                      <span v-if="open" key="0"> Bill Detail</span>
                      <span v-else key="1">
                        <v-chip :color="getColor(item.billItems.length)" dark>
